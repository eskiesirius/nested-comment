<template>
	...
	<section aria-labelledby="comments" class="px-4 sm:px-6 lg:px-8 bg-gray-50">
		<div class="px-4 pt-6 sm:px-6">
			<h1 class="text-lg font-bold mb-3">Leave A Comment</h1>
			<div class="flex space-x-3">
				<div class="min-w-0 flex-1">
					...
					<div class="mt-2">
						<textarea id="comment" name="comment" rows="3" class="shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 sm:text-sm border border-gray-300 rounded-md" placeholder="Leave a comment here..." v-model="form.body" />
						<ErrorMessage class="mt-2" :message="form.errors.body" />
					</div>
					<div class="mt-3 flex items-center justify-end">
						<button type="submit" class="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" @click="submit" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Comment</button>
					</div>
				</div>
			</div>
		</div>
		<div class="px-4 pb-6 sm:px-6">
			...
			<ul role="list" class="space-y-4">
				<ReplyList v-for="comment in comments.data" :comment="comment" :key="comment.id" />
			</ul>
		</div>
	</section>
</template>
